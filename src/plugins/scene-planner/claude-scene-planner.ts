...
IMPORTANT: ALL images are AI-generated. There are NO stock photos. Every visual description you write will be sent to an AI image generator (FLUX). Write vivid, specific visual descriptions for EVERY scene type.

AVAILABLE SCENE TYPES:

1. "real_photo" — Photorealistic AI image of a SPECIFIC real thing.
   The AI will generate a realistic-looking depiction that looks like a real photograph.
   USE FOR: specific real things — named animals/species, real places/landmarks,
   physical objects, food, vehicles, buildings, specific technology, famous experiments.
   DESCRIPTION: Write a detailed visual description (20-40 words) of exactly what
   the image should show. Include setting, lighting, perspective, camera angle.
   Example: "The Hubble Space Telescope floating in orbit above Earth, golden solar panels extended, deep black space dotted with stars behind it, sunlight glinting off its silver body"

2. "ai_illustration" — Cartoon/educational style illustration.
   Realistic depictions of things drawn in a fun, educational cartoon style.
   NOT abstract art — think textbook illustration or educational poster.
   USE FOR: theoretical concepts, microscopic/atomic scale, far future scenarios,
   artistic interpretations, "imagine this" moments, cutaway views, abstract ideas.
   DESCRIPTION: Describe the SCENE (20-40 words), include style and mood.
   Example: "A glowing translucent human brain with neural pathways lighting up in electric blue, surrounded by floating mathematical equations, clean light background"

3. "cinematic_ai" — Dramatic photorealistic movie-quality shot.
   USE SPARINGLY for maximum impact — 1 per section at most.
   USE FOR: opening dramatic shot of a section, key visual climax, epic scale.
   DESCRIPTION: Describe as a movie shot (20-40 words), include camera angle and drama.
   Example: "Aerial cinematic shot of a massive volcanic eruption at sunset, lava flowing into the ocean creating enormous steam clouds, dramatic orange and red sky"

4. "diagram" — Hand-drawn whiteboard diagram (auto-generated by Claude as SVG).
   Looks like someone drew it on a whiteboard with colored markers.
   USE FOR: step-by-step processes, size comparisons, timelines, statistics, data.
   DESCRIPTION: What to SHOW + layout + what text labels are needed.

5. "text_card" — Big bold text on screen. Maximum 12 words. Use sparingly (max 1 per section).
...
[
  {
    ...
    "ai_image_prompt": "detailed visual description for AI image generation (ALL scene types except diagram/text_card)",
...
RULES:
- Use real_photo for 30-40% of scenes (realistic depictions of specific things)
- Use ai_illustration/cinematic_ai for 40-50% (concepts, drama, abstract ideas)
- Use cinematic_ai SPARINGLY — max 1 per section for dramatic impact
- Use diagram for 0-15%, text_card for 0-10%
- ai_image_prompt is REQUIRED for real_photo, ai_illustration, and cinematic_ai
- Write DETAILED visual descriptions (20-40 words) — the AI needs specifics to generate good images
...
export async function planAllSections(
  ...
  for (const topic of script.subTopics) {
    ...
    try {
      ...
      console.warn(`[scene-planner-v2]   ⚠️ Failed for "${topic.name}": ${(err as Error).message}`);
      // Fallback: single cinematic AI scene for the whole section
      sectionPlans.set(topic.index, [{
        scene_id: `s${topic.index}_0`,
        scene_type: "cinematic_ai",
        narration_text: topic.narrationText,
        duration_estimate_seconds: Math.max(topic.narrationText.split(/\s+/).length / 2.5, 5),
        image_search_query: "",
        ai_image_prompt: `Dramatic cinematic visualization of ${topic.name}, detailed, dark background, vivid colors`,
        camera: { type: "zoom_in" },
        mood: "mysterious",
      }]);
    }
  ...
  if (script.hook) {
    ...
    try {
      const hookScenes = await planSectionScenes(script.hook, -1, "_hook", 1, 3);
      sectionPlans.set(-1, hookScenes);
      console.log(`[scene-planner-v2]   → ${hookScenes.length} hook scenes`);
    } catch (err) {
      console.warn(`[scene-planner-v2]   ⚠️ Hook failed: ${(err as Error).message}`);
      sectionPlans.set(-1, [{
        scene_id: "hook_0",
        scene_type: "cinematic_ai",
        ...
        duration_estimate_seconds: 8,
        image_search_query: "",
        ai_image_prompt: script.hook.slice(0, 120),
  ...
  if (script.outro) {
    ...
    try {
      const outroScenes = await planSectionScenes(script.outro, -2, "_outro", 1, 2);
      sectionPlans.set(-2, outroScenes);
      console.log(`[scene-planner-v2]   → ${outroScenes.length} outro scenes`);
    } catch (err) {
      console.warn(`[scene-planner-v2]   ⚠️ Outro failed: ${(err as Error).message}`);
      sectionPlans.set(-2, [{
        scene_id: "outro_0",
        scene_type: "cinematic_ai",
